...
- **Event Routing Fix:** Includes a workaround for Morphic's `processMouseUp` event routing. Drag events released over child morphs (blocks) are forced to fire the background's `mouseClickLeft` sequence, which resolves ghost rendering artifacts.
